...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (type === 'contact') {
      ...
      if (res?.status) return res.status(200).json({ success: true });
    ...
    if (res?.status) return res.status(200).json({ success: true, warning: error?.message });
    return new Response(JSON.stringify({ success: true, warning: error?.message }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }
}
